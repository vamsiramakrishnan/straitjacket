...
This is a dated directional receipt, not a standing regression gate. Each arm
has two repeats, the agent host does not expose a controllable temperature or
seed, and the committed record contains aggregates rather than every raw
transcript. Treat the heavy-flood effect as the useful signal and the
low-volume deltas as regime evidence, not as precise performance estimates.
Any current headline claim should be rerun with frozen fixtures, more repeats,
raw per-run records, and the current model/runtime before it is promoted.
