...
| A nested web/MCP call returned a ctx digest as a tool error despite the captured command exiting 0 | successful external evidence looked failed or risked changing type inside code mode | **fixed in source v0.33:** Codex string replacement now uses success-preserving `continue:false`; structured arrays/objects pass through unchanged while full CallToolResult envelopes retain `content`, `isError`, and `structuredContent` in capture. Direct structured containment remains a gateway concern until Codex supports `updatedMCPToolOutput` |
| Programmatic code can deliberately print an entire string result after the nested tool succeeds | a caller can recreate context bloat after the inner birth gate | **open, P1:** route high-volume programmatic tools through `ctx surface gateway`, and add a host-supported outer program-output gate when Codex exposes that lifecycle boundary; meanwhile programs must reduce before emitting `text()` |
