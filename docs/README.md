...
| 5 | [EVIDENCE-PLANS.md](EVIDENCE-PLANS.md) | Compiled evidence plans: the model compiles exploration intent into a typed, total, bounded DAG (`ctx plan` / `ctx investigate`); ast-grep and Semgrep join as physical operators behind logical ops; rounds go from O(operations) to O(hypothesis epochs). Shipped v0.25.0. |
