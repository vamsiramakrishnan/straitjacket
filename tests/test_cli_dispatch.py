...
def test_workspace_free_commands_are_exactly_the_self_resolving_ones():
    ...
    must work outside one); proxy and replay need none at all; image reads the
    file paths it is given and never touches a workspace. Everything else —
    orchestrate included — gets its workspace from the dispatcher."""
    from ctx import cli

    free = {n for n, (_, _, ws) in cli._COMMANDS.items() if not ws}
    assert free == {"wrap", "antigravity", "proxy", "replay", "image"}
