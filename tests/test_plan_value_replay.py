"""Acceptance: ctx replay --outcomes — deterministic evidence_followup/v1
...
def _session(tmp_path):
    ...
    entries = [
        ...
        # follows up on the emitted handle with an addressed slice
...
def test_followups_from_transcript(tmp_path):
    p = _session(tmp_path)
    events = session_outcomes(p)
    assert events, "expected follow-up events"
    by_op = {}
    for e in events:
        by_op.setdefault(e.operator, []).append(e)
    (pytest_ev,) = by_op["profile:pytest/v2"]
    assert pytest_ev.used_exactly
    assert {"exact_handle", "exact_test_id"} <= set(pytest_ev.match_classes)
    ...
    assert all(not e.used_exactly for e in last)


def test_followups_deterministic_and_read_only(tmp_path):
    p = _session(tmp_path)
    before = p.read_bytes()
...
def test_scoreboard_counts_not_rates(tmp_path):
    events = session_outcomes(_session(tmp_path))
    board = render_outcomes(events)
    assert "Operator follow-up (association, not causation)" in board
    assert "profile:pytest/v2" in board
    assert "censored" in board
    assert "counts, not rates" in board
    assert "%" not in board  # the scoreboard never renders bare percentages
    payloads = [e.payload() for e in events]
    assert all("match_classes" in p and "censored" in p for p in payloads)
