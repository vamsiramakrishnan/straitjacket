...
def _png_header(w: int, h: int, color_type: int = 2) -> bytes:
    """Enough of a PNG IHDR for the dependency-free structural parser."""
    return (
        b"\x89PNG\r\n\x1a\n"
        + (13).to_bytes(4, "big")
        + b"IHDR"
...
def test_png_dimensions_stdlib():
    info = binfmt.inspect(_png_header(320, 240))
...
@pytest.mark.skipif(not _HAS_PIL, reason="image extra (Pillow) not installed")
...
def test_pdf_page_count_and_text_stdlib():
    ...
    assert info.pages == 2
    assert info.text_operator_seen is True
    rendered = binfmt.render_digest(info)
    assert "visible page objects: 2 (heuristic)" in rendered
    assert "text operators: visible" in rendered


def test_digest_never_contains_pixels():
    data = _png_header(200, 150) + b"\x00" * 2_000
    d = binfmt.render_digest(binfmt.inspect(data), address="ctx get run:x#stdout")
    assert "image: 200×150" in d
    assert "phash: unavailable" in d
...
def test_ctx_run_binary_profile_end_to_end():
    with tempfile.TemporaryDirectory() as td:
        (Path(td) / "shot.png").write_bytes(_png_header(320, 240))
        proc = subprocess.run(
            [
                sys.executable, "-m", "ctx", "--workspace", td,
                "run", "--shell", "cat shot.png",
            ],
    ...
    assert "phash: unavailable" in out
...
def test_ctx_image_diff_cli():
    with tempfile.TemporaryDirectory() as td:
        ...
        r = subprocess.run([sys.executable, "-m", "ctx", "--workspace", td,
                            "image", "diff", str(a), str(b)],
                           capture_output=True, text=True, env=env)
        assert r.returncode == 0
        assert "dhash distance:" in r.stdout
        r2 = subprocess.run([sys.executable, "-m", "ctx", "--workspace", td,
                             "image", "diff", str(a), str(a)],
                            capture_output=True, text=True, env=env)
        assert "identical dHash" in r2.stdout and "byte identity: equal" in r2.stdout
