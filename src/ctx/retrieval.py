...
def _resolve_span(store: Store, ws: Workspace, ref_text: str, label: str, span_id: str) -> str:
    """Resolve a digest-minted span token to a bounded view (SPEC §6.4).

    The invariant that beats raw-refill retrieval: resolution can never
    flood. Small regions return exact lines; large regions return a zoom
    sub-digest (template mining over just that slice) minting fresh
    sub-spans; template spans return a bounded occurrence listing. Every
    level declares its omission and hands out coordinates.
    """
    budget = ws.config.budgets
